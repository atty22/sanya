        ...
        for word in available_keys:
            if word in event.raw_text.lower():
                if len(available_keys[word]) !=3:
                    _mood=0
                (await (event.reply(available_keys[word][( 0 if len(available_keys[word]) != 3 else mood())]))
                 if available_keys[word][( 0 if len(available_keys[word]) !=3 else mood())] != "" else None)
                return
